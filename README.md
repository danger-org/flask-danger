...
- **Country and timezone resolution:** Get the user's country and timezone, with built-in fallback so your sign ups are never broken.
- **Validate user details:** Validate and normalize user-provided inputs. Check email deliverability, disposable domains, phone number reachability, parse addresses, and much more. Simple `valid` and `normalized` properties take the complication away.
...
The result of this server-side call will provide a simple `allow` or `block` action, and give your app access to enriched data about the user and their device.
...
- **`DANGER_FALLBACK_ALLOW`**: Optional. Where to allow or block if the Danger platform can't be reached. Set to either `True` (allow) or `False` (block). Defaults to `True` so that the check fails open.
...
```python
    ...
    if result.phone_valid is False:
        # Phone is definitely not valid
    if result.address_valid is False:
        # Address is definitely not valid
```
...
| country       | The 2-letter country code for the user (e.g. `'US'`)                                                                                                                                           |
